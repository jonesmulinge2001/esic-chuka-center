...
@Component({
  ...
  styles: [
    `
      ...
      .btn-primary:hover {
        transform: translateY(-1px);
        box-shadow: 0 4px 14px rgba(0, 51, 153, 0.32) !important;
      }
  ...
  template: `
    <header
      class="bg-white border-b border-[#e5e8f0] shadow-[0_1px_3px_rgba(0,20,80,.06),0_1px_8px_rgba(0,20,80,.04)] animate-slide-down"
    >
      <div class="max-w-7xl mx-auto px-4 sm:px-5 lg:px-8">
        <div class="flex items-center justify-between h-14">
          ...
          <a
            routerLink="/"
            (click)="scrollToTop()"
            class="group/logo flex items-center gap-2.5 flex-shrink-0"
          >
            <div
              class="logo-icon w-[34px] h-[34px]
            ...
            <div class="hidden sm:block">
              <div
                class="font-extrabold text-[11.5px] text-[#001e5c] leading-none tracking-[.06em] uppercase"
              >
                ESIC
              </div>
              <div class="text-[9px] text-slate-400 leading-tight mt-0.5">
                Electronics &amp; Software Innovation Center
              </div>
            </div>
          </a>
          ...
          <nav class="hidden lg:flex items-center gap-0.5">
            @for (link of navLinks; track link.label) { @if (link.children) {
            <div class="relative group/dd">
              <button
                class="flex items-center gap-0.5 px-2.5 py-[5px] rounded-[7px]
              ...
              >
                {{ link.label }}
                <span class="material-icons chevron text-[13px] opacity-70"
                  >expand_more</span
                >
              </button>
              <div
                class="dd-menu absolute top-full left-0 mt-1.5 min-w-[184px]
          ...
          <div class="flex items-center gap-1.5">
            ...
            >
              <span class="material-icons text-[15px] text-[#003399]"
                >account_circle</span
              >
              {{ auth.user()?.firstName }}
            </a>
            @if (auth.isAdmin()) {
            <span
              class="hidden sm:inline text-[10px] font-bold px-2 py-0.5
        ...
        @if (menuOpen()) {
        <nav
          class="animate-mobile-slide lg:hidden border-t border-slate-100 py-2 pb-3 flex flex-col gap-0.5"
        >
          @for (link of navLinks; track link.label) { @if (!link.children) {
          <a
            [routerLink]="link.path"
            (click)="scrollToTop(); menuOpen.set(false)"
            routerLinkActive="text-[#003399] bg-[#eff4ff]"
          ...
          <div
            class="px-3.5 pt-3 pb-0.5 text-[9.5px] font-bold
  ...
  navLinks: Array<{
    ...
    { label: 'About ESIC', path: '/about' },
    {
      ...
      children: [
        ...
        {
          label: 'Engineering Laboratory Trainers',
          path: '/programs/junior-stem',
        },
        { label: 'STEM Classroom Solutions', path: '/programs/advanced-stem' },
        {
          label: 'Industrial Training',
          path: '/programs/lab-industrial-training',
        },
      ],
    },
    {
